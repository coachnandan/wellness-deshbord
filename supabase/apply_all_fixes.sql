...
-- 7. Create visitor_shake_logs table for Standalone Visitor Shakes
...
-- Policies
CREATE POLICY "Authenticated users can select visitor_shake_logs" ON public.visitor_shake_logs FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can insert visitor_shake_logs" ON public.visitor_shake_logs FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can update visitor_shake_logs" ON public.visitor_shake_logs FOR UPDATE USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can delete visitor_shake_logs" ON public.visitor_shake_logs FOR DELETE USING (auth.role() = 'authenticated');
...
DO $$$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'visitor_shake_logs') THEN
